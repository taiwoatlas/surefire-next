export function StageCurtain({ side = "left" }: { readonly side?: "left" | "right" }) {
  ...
  return (
    <svg
      ...
      className={`pointer-events-none absolute inset-y-0 ${side === "left" ? "left-0" : "right-0"} z-0 h-full w-20 md:w-32 ${flip}`}
    >
      <defs>
        <linearGradient id={`curtainFold-${side}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="#9C3151" stopOpacity="1" />
          <stop offset="55%" stopColor="#6E1F32" stopOpacity="0.95" />
      ...
      <path d="M0,0 C 40,40 80,20 110,55 C 140,20 180,40 220,0" stroke="#D6A647" strokeWidth="3" fill="none" opacity="0.9" />
    </svg>
  );
}
